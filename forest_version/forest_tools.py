...
def init_qvm_and_quilc(qvm_executable="qvm", quilc_executable="quilc"):
    ...
    fc = ForestConnection(
        sync_endpoint=f'http://127.0.0.1:{str(qvm_port)}',
        compiler_endpoint=f'http://127.0.0.1:{str(quilc_port)}',
    )
...
def plot_circuit(circuit):
    ...
    tmp_folder = mkdtemp()
    with open(f'{tmp_folder}/circuit.tex', 'w') as f:
        f.write(latex_diagram)
    proc = subprocess.Popen(
        ['pdflatex', '-shell-escape', f'{tmp_folder}/circuit.tex'],
        cwd=tmp_folder,
    )
    proc.communicate()
    image = plt.imread(f'{tmp_folder}/circuit.png')
...
def get_vector(alpha, beta):
    ...
    if abs(alpha) == 0 or abs(beta) == 0:
        return [0,0,-1] if alpha == 0 else [0,0,1]
...
def plot_histogram(result):
    outcomes = np.vstack(result.values()).T if isinstance(result, dict) else result
    trials, classical_bits = outcomes.shape
    stats = {
        "".join(str(bit) for bit in bits): 0
